...
from checker import Checker # New import
...
def compile_source(source_code, file_path):
    ...
    # 2.5. Checker (Semantic Analysis)
    checker = Checker(file_path, source_code.split('\n'))
    checker.check(ast)
...
def main():
    ...
    arg_parser = argparse.ArgumentParser(prog="ignis", description="The Ignis language compiler.", epilog="Have fun building the future!")
    arg_parser.add_argument('input_file', type=str, help='The Ignis source file to compile (e.g., ../examples/test.ign)')
    arg_parser.add_argument('-o', '--output', type=str, help='Specify the output file name. Defaults to the input file name without extension.')
    arg_parser.add_argument('-S', action='store_true', help='Stop after the compilation stage; do not assemble. (Generates a .asm file)')
    arg_parser.add_argument('-c', action='store_true', help='Stop after the assembling stage; do not link. (Generates a .o file)')
    arg_parser.add_argument('-k', '--keep-files', action='store_true', help='Keep intermediate files (.asm, .o) after compilation.')
    ...
    try:
        ...
        with open(asm_file_path, 'w') as f: f.write(assembly_code)
        ...
        print("\nError: 'nasm' or 'ld' not found. Please ensure they are in your PATH."); sys.exit(1)
