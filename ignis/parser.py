# ==============================================================================
# File: ignis/parser.py (FIXED assignment to dereferenced pointers)
# ==============================================================================
...
class Parser:
    ...
    def type_spec(self):
        ...
        self.error("Expected a base type specifier (e.g., 'int')")
    ...
    def for_statement(self):
        ...
        if self.current_token.type != TokenType.SEMICOLON:
            if self.current_token.type in (TokenType.KW_INT, TokenType.KW_MUT, TokenType.KW_PTR):
                init_node = self.variable_declaration()
            else:
                init_node = self.assignment_statement()
    ...
    def assignment_statement(self, left_node):
        """Parses the right side of an assignment, given the left side."""
        op = self.current_token
        self.eat(TokenType.ASSIGN)
        right = self.expr()
    ...
    def statement(self):
        ...
        if token_type in (TokenType.KW_INT, TokenType.KW_MUT, TokenType.KW_PTR):
            ...
            # This is the key change. We parse an expression first...
            node = self.expr()
            # ...and then check if it's followed by an assignment.
            if self.current_token.type == TokenType.ASSIGN:
                if not isinstance(node, (Var, UnaryOp)):
                    self.error("The left-hand side of an assignment must be a variable or a dereference.")
    ...
    def declaration(self):
        if self.current_token.type == TokenType.KW_CONST:
            ...
            self.eat(TokenType.SEMICOLON);
            return node
        if self.current_token.type in (TokenType.KW_INT, TokenType.KW_PTR):
            type_node = self.type_spec();
