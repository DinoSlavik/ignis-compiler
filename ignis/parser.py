...
class Parser:
    ...
    def factor(self):
        ...
        if token.type == TokenType.INTEGER:
            ...
            # A full if-elif-else block is a factor
    ...
    def expr(self):
        ...
        It now correctly handles the ternary operator's precedence.
        """
        # First, parse the left-hand side, which could be a full comparison.
        node = self.comparison_expr()

        # Then, check if it's followed by a ternary 'if'.
        if self.current_token.type == TokenType.KW_IF:
            ...
            else_expr = self.expr()  # Recursive call for nested ternaries

            # Wrap results in blocks to be consistent with standard if-expressions
            if_block = Block()
