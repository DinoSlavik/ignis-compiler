# 5. Складені типи даних: Структури, Класи та Вказівники
Ignis надає два основні інструменти для створення власних, складних типів даних: **структури** та **класи**. 
Вони дозволяють групувати дані та логіку, а **вказівники** забезпечують гнучке керування пам'яттю для роботи з ними.

## Структури (`struct`)
Структури — це простий та ефективний спосіб згрупувати кілька змінних (полів) в один логічний тип. 
**Основне призначення структур — зберігання даних**. Вони не містять пов'язаної з ними логіки (методів).
...
## Класи (`class`)
На відміну від структур, класи дозволяють **інкапсулювати** — об'єднувати в єдине ціле — як дані (поля), 
так і логіку для роботи з цими даними (методи). Це основний будівельний блок для об'єктно-орієнтованого програмування (ООП) в Ignis.

### Оголошення
Синтаксис оголошення класу схожий на структури, але використовує ключове слово `class` та може включати модифікатори доступу (`public`, `private`).

```Ignis

// Попередній вигляд синтаксису
class Player {
    public:
...
_Примітка: детальніше читайте у розділі про ООП._
